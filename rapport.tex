...
\caption{Heatmap des poids par classe (rouge = positif, bleu = négatif)}
\end{figure}
...
\caption{Exemple de prédictions (vert = correct, rouge = erreurs)}
\end{figure}
...
L'implémentation from scratch a nécessité une compréhension approfondie des mathématiques sous-jacentes, notamment pour la gestion des gradients et l'extension à la classification multiclasse via One-vs-Rest. Le choix des hyperparamètres s'est fait par essais-erreurs, faute de validation croisée automatisée. La comparaison entre Digits (8x8) et MNIST (28x28) reste délicate en raison de leurs différences de complexité. Enfin, l'entraînement from scratch s'avère nettement plus lent que les implémentations optimisées de scikit-learn.

\subsection{Limites du Travail}

La régression logistique, limitée aux séparations linéaires, ne capture pas les non-linéarités complexes des chiffres manuscrits. Le dataset Digits, avec seulement ~1800 échantillons, reste modeste, tandis que MNIST nécessiterait des modèles plus sophistiqués. L'absence de validation croisée systématique rend les résultats dépendants de la séparation train/test choisie. On observe également un compromis entre interprétabilité et performance : la régression logistique reste explicable mais moins efficace que les réseaux de neurones sur des tâches visuelles complexes.

\subsection{Perspectives d'Amélioration}

Plusieurs pistes permettraient d'améliorer ce travail : l'utilisation de techniques de preprocessing avancées comme l'augmentation de données, l'implémentation de modèles plus sophistiqués (SVM, Random Forest, CNN), l'ajout systématique de validation croisée, et l'extension à d'autres datasets pour tester la généralisation des modèles.

\section{Conclusion}

Ce projet illustre la progression de modèles simples vers des approches plus avancées. La régression logistique from scratch atteint une précision de 94-97\% sur Digits, démontrant l'efficacité d'une implémentation manuelle bien maîtrisée. Les réseaux de neurones, malgré leur architecture simple, atteignent ~92\% sur MNIST. Les erreurs observées soulignent l'importance des représentations non-linéaires pour les tâches visuelles complexes. Ce travail renforce notre compréhension des algorithmes d'apprentissage automatique et des compromis entre interprétabilité, performance et complexité.
